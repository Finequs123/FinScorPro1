  ...
  const runSimulation = async () => {
    if (!file || !selectedScorecard) {
      toast({
        ...
        description: "Please select a scorecard and upload a data file",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      const data = lines.slice(1).map((line, index) => {
        ...
        const record: any = { id: `ROW_${index + 1}` };
        headers.forEach((header, headerIndex) => {
          if (header && values[headerIndex] !== undefined) {
            record[header] = values[headerIndex]?.trim().replace(/"/g, '') || '';
          }
        });
        return record;
      ...
      toast({
        ...
        description: error.message || "Failed to process simulation",
        variant: "destructive"
      });
    }
  };
