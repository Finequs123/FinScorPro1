...
export default function ScorecardConfig() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const response = await apiRequest("PUT", `/api/scorecards/${id}`, {
        ...data,
  ...
  const handleScorecardSelect = (scorecard: Scorecard) => {
    ...
    try {
      let parsedConfig;
      if (scorecard.configJson) {
        parsedConfig = typeof scorecard.configJson === 'string' 
          ? JSON.parse(scorecard.configJson) 
          : scorecard.configJson;
        
        // Normalize config structure
        const normalizedConfig = {
          categories: parsedConfig.categories || defaultConfig.categories,
          bucketMapping: parsedConfig.bucketMapping || parsedConfig.buckets || defaultConfig.bucketMapping,
          rules: parsedConfig.rules || defaultConfig.rules,
          metadata: parsedConfig.metadata || defaultConfig.metadata
        };
        ...
        validateWeights(defaultConfig);
      }
    } catch (error) {
      console.error("Failed to parse scorecard config:", error);
      setConfig(defaultConfig);
      validateWeights(defaultConfig);
      toast({
        title: "Configuration Warning",
        description: "Loaded default configuration due to parsing error",
        variant: "destructive"
      });
    }
